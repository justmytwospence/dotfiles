...
** Linux
   These are for a local Linux desktop.
...
** OSX
   These are for a primary OSX computer.

   - [[http://www.hammerspoon.org/][hammerspoon]]
   - [[http://www.launchd.info/][Launchd]]

** local
   These are for any primary local computer.
...
** common
   These are for any machine, remote or local.
...
* Setup
  ...
  #+BEGIN_SRC sh :tangle no
  $HOME/dotfiles/$layer/.local/bin/bootstrap-$layer
  #+END_SRC

** Linux
   :PROPERTIES:
   :header-args+: :tangle linux/.local/bin/bootstrap-linux
   :END:

   Restore repositories:

   #+BEGIN_SRC sh
   sudo cp -r $HOME/dotfiles/linux/.config/apt/sources.list.d/* /etc/apt/sources.list.d
   #+END_SRC
...
** OSX
   :PROPERTIES:
   :header-args+: :tangle osx/.local/bin/bootstrap-osx
   :END:
...
** common
   :PROPERTIES:
   :header-args+: :tangle common/.local/bin/bootstrap-common
   :END:
...
** local
   :PROPERTIES:
   :header-args+: :tangle local/.local/bin/bootstrap-local
   :END:
